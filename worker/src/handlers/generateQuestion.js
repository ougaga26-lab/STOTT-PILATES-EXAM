...
export async function handleGenerateQuestion(request, env, corsHeaders) {
  ...
  try {
    ...
    if (err instanceof GeminiError) {
      ...
      return corsResponse(status, { error: { code: 'GEMINI_ERROR', message: 'AI service error. Please try again.' } }, corsHeaders);
    }
  ...
  try {
    ...
    return corsResponse(500, { error: { code: 'PARSE_ERROR', message: 'AI returned unexpected format. Please try again.' } }, corsHeaders);
  }
